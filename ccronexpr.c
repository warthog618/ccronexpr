...
static const char *const MONTHS_ARR[] = {"FOO", "JAN", "FEB", "MAR", "APR", "MAY", "JUN",
...
#define CRON_ERROR(message)                                                                                            \
    {                                                                                                                  \
        *error = message;                                                                                              \
        goto error;                                                                                                    \
    }
#define TOKEN_COMPARE(context, token)                                                                                  \
    if (context->err)                                                                                                  \
        goto error;                                                                                                    \
...
#define MKTIME(calendar)                                                                                               \
    if (CRON_INVALID_INSTANT == cron_mktime(calendar))                                                                 \
        goto return_error;
#define STRCATC(dest, buf, inc_len)                                                                                    \
    do {                                                                                                               \
        len += inc_len;                                                                                                \
        if (len > buffer_len)                                                                                          \
            return -1;                                                                                                 \
...
#define GFC(dest, bits, min, max, offset, overflow, buffer_len)                                                        \
    {                                                                                                                  \
        tmp = generate_field(dest, bits, min, max, offset, overflow, buffer_len);                                      \
        if (tmp < 0)                                                                                                   \
            return tmp;                                                                                                \
        else                                                                                                           \
            len += tmp;                                                                                                \
...
static int next_set_bit(uint8_t *bits, int max, int from_index) {
    for (; from_index < max; from_index++)
        if (cron_get_bit(bits, from_index))
            return from_index;
    return -1;
}
...
static int last_day_of_month(int month, int year, int is_weekday) {
    ...
    if (is_weekday) {
        /* If the last day of the month is a Saturday (6) or Sunday (0), decrement the day. But it is shifted to (5)
         * and (6). */
        while (cron_time(&t, &calendar)->tm_wday == 6 || cron_time(&t, &calendar)->tm_wday == 0)
...
static int closest_weekday(int day_of_month, int month, int year) {
    ...
    if (wday == 0) {
        ...
        if (day_of_month + 1 == last_day_of_month(month, year, 0))
            t -= 2 * DAY_SECONDS;
        else
 ...
 */
static void reset_min(struct tm *calendar, int field) { set_field(calendar, field, field == CRON_CF_DAY_OF_MONTH); }
static void reset_max(struct tm *calendar, int field) {
    if (CRON_CF_SECOND == field)
        set_field(calendar, field, CRON_MAX_SECONDS - 1);
    else if (CRON_CF_MINUTE == field)
        set_field(calendar, field, CRON_MAX_MINUTES - 1);
    else if (CRON_CF_HOUR_OF_DAY == field)
        set_field(calendar, field, CRON_MAX_HOURS - 1);
    else if (CRON_CF_DAY_OF_MONTH == field)
        set_field(calendar, field, last_day_of_month(calendar->tm_mon, calendar->tm_year, 0));
    else if (CRON_CF_MONTH == field)
        set_field(calendar, field, CRON_MAX_MONTHS - 1);
}
...
typedef struct {
    ...
    TokenType type;
    cron_expr *target;
    int field_type, value, min, max, offset, fix_dow;
    uint8_t *field;
    const char *err;
} ParserContext;
...
static int match_ordinals(const char *str, const char *const *arr, size_t arr_len) {
    size_t i;
    for (i = 0; i < arr_len; i++)
        if (!compare_strings(str, arr[i], strlen(arr[i])))
            return (int)i;
    return -1;
}
...
static void token_next(ParserContext *context) {
    ...
    if (*context->input == '\0')
        ...
        if (context->value < 0)
            context->value = match_ordinals(context->input, MONTHS_ARR, CRON_MONTHS_ARR_LEN);
        if (context->value < 0)
            goto rest;
        context->input = input;
        context->type = T_NUMBER;
    ...
    rest:
        ...
        case '*':
            context->type = T_ASTERISK;
            break;
        case '?':
            context->type = T_QUESTION;
            break;
        case ',':
            context->type = T_COMMA;
            break;
        case '/':
            context->type = T_SLASH;
            break;
        case 'L':
            context->type = T_L;
            break;
        ...
        case '#':
            ...
            break;
        case '-':
            context->type = T_MINUS;
            break;
        }
        ++context->input;
    }
    if (T_INVALID == context->type)
        context->err = "Invalid token";
}
...
static int Number(ParserContext *context) {
    ...
    case T_MINUS:
        ...
        if (T_NUMBER == context->type) {
            ...
            PARSE_ERROR("Number '-' follows with number");
        break;
    case T_NUMBER:
        value = context->value;
        token_next(context);
...
static int Frequency(ParserContext *context, int delta, int *to, int range) {
    ...
    case T_SLASH:
        ...
        if (T_NUMBER == context->type) {
            delta = context->value;
            if (delta < 1)
                PARSE_ERROR("Frequency - needs to be at least 1");
            if (!range)
                *to = context->max - 1;
            token_next(context);
        } else
            PARSE_ERROR("Frequency - '/' follows with number");
        break;
    case T_COMMA:
    case T_WS:
    case T_EOF:
    ...
    case T_HASH:
        if (CRON_CF_DAY_OF_WEEK == context->field_type) {
            ...
            *context->target->day_in_month = (int8_t)Number(context);
            ...
            PARSE_ERROR("Nth-day - '#' allowed only for day of week");
        break;
    case T_MINUS:
        token_next(context);
        if (T_NUMBER == context->type) {
            ...
            PARSE_ERROR("Range '-' follows with number");
        break;
    case T_W:
        *context->target->day_in_month = (int8_t)to;
        *from = context->min;
        for (i = 1; i <= 5; i++)
            cron_set_bit(context->target->days_of_week, i);
    ...
    case T_L:
        if (CRON_CF_DAY_OF_WEEK == context->field_type) {
            *context->target->day_in_month = -1;
            ...
            PARSE_ERROR("Range - 'L' allowed only for day of week");
        break;
    case T_WS:
    case T_SLASH:
    case T_COMMA:
...
start:
    ...
    case T_ASTERISK:
        token_next(context);
        delta = Frequency(context, delta, &to, 0);
        break;
    case T_NUMBER:
        from = context->value;
        token_next(context);
    ...
    case T_L:
        ...
        case CRON_CF_SECOND:
            if (!leap)
                context->max += CRON_MAX_LEAP_SECONDS;
            leap = 1;
            goto start;
        case CRON_CF_DAY_OF_MONTH:
            *context->target->day_in_month = -1;
            ...
            case T_MINUS:
            case T_NUMBER:
                *context->target->day_in_month = (int8_t)((int)*context->target->day_in_month + Number(context));
                break;
            case T_W:
                if (CRON_CF_DAY_OF_MONTH == context->field_type) {
                    ...
                    PARSE_ERROR("Offset - 'W' allowed only for day of month");
            case T_COMMA:
            case T_WS:
            case T_EOF:
                break;
            default:
                PARSE_ERROR("Offset - error");
            }
        ...
        case CRON_CF_DAY_OF_WEEK:
            ...
            break;
        default:
            PARSE_ERROR("Segment 'L' allowed only for day of month and leap seconds")
        }
        break;
    case T_W:
        for (i = 1; i <= 5; i++)
    ...
    case T_QUESTION:
        ...
        break;
    default:
        PARSE_ERROR("Segment - error");
    }
done:
    if (context->err)
        goto error;
    if (CRON_CF_DAY_OF_WEEK == context->field_type && context->fix_dow)
        return;
    if (from < context->min || to < context->min)
        PARSE_ERROR("Range - specified range is less than minimum");
    if (from >= context->max || to >= context->max)
        PARSE_ERROR("Range - specified range exceeds maximum");
    if (from > to)
        PARSE_ERROR("Range - specified range start exceeds range end");
    for (; from <= to; from += delta)
        cron_set_bit(context->field, from + context->offset);
    if (CRON_CF_DAY_OF_WEEK == context->field_type) {
...
static void Fields(ParserContext *context, int len) {
    token_next(context);
    if (len < 6)
        cron_set_bit(context->target->seconds, 0);
    else {
    ...
    if (len < 7)
        cron_set_bit(context->target->years, EXPR_YEARS_LENGTH * 8 - 1);
    else {
        TOKEN_COMPARE(context, T_WS);
    ...
    return;
compare_error:
    PARSE_ERROR("Fields - expected whitespace separator");
error:
    return;
}
 ...
 */
static int find_nextprev(uint8_t *bits, int max, int value, int value_offset, struct tm *calendar, int field,
                         int nextField, uint8_t *lower_orders, int offset) {
    int next_value =
        (offset > 0 ? next_set_bit(bits, max, value + value_offset) : prev_set_bit(bits, value + value_offset, 0));
    ...
    if (next_value < 0) {
        if (offset > 0)
            reset_max(calendar, field);
        else
            reset_min(calendar, field);
    ...
    if (next_value < 0 || next_value != value) {
        if (offset > 0)
            reset_all_min(calendar, lower_orders) else reset_all_max(calendar, lower_orders);
        set_field(calendar, field, next_value < 0 ? 0 : next_value);
        MKTIME(calendar);
    }
    return next_value < 0 ? 0 : next_value;
return_error:
    return -1;
}

static int find_day_condition(struct tm *calendar, uint8_t *days_of_month, int8_t *dim, int dom, uint8_t *days_of_week,
                              int dow, uint8_t *flags, int *day) {
    int tmp_day = *day;
    if (tmp_day < 0) {
        if ((!*flags && *dim < 0) || *flags & 1)
            tmp_day = last_day_of_month(calendar->tm_mon, calendar->tm_year, 0);
        else if (*flags & 2)
            tmp_day = last_day_of_month(calendar->tm_mon, calendar->tm_year, 1);
        else if (*flags & 4)
            tmp_day = closest_weekday(*dim - 1, calendar->tm_mon, calendar->tm_year);
        *day = tmp_day;
    }
    if (!cron_get_bit(days_of_month, dom) || !cron_get_bit(days_of_week, dow))
        return 1;
    if (*flags) {
        if ((*flags & 3) && dom != tmp_day + 1 + *dim)
            return 1;
        if ((*flags & 4) && dom != tmp_day)
            return 1;
    } else {
        if (*dim < 0 && (dom < tmp_day + WEEK_DAYS * *dim + 1 || dom >= tmp_day + WEEK_DAYS * (*dim + 1) + 1))
            return 1;
        if (*dim > 0 && (dom < WEEK_DAYS * (*dim - 1) + 1 || dom >= WEEK_DAYS * *dim + 1))
            return 1;
    }
    return 0;
}

static int find_day(struct tm *calendar, uint8_t *days_of_month, int8_t *dim, int dom, uint8_t *days_of_week, int dow,
                    uint8_t *flags, uint8_t *resets, int offset) {
    ...
    while (find_day_condition(calendar, days_of_month, dim, dom, days_of_week, dow, flags, &day) && count++ < max) {
        if (offset > 0)
            reset_all_min(calendar, resets) else reset_all_max(calendar, resets);
        add_to_field(calendar, CRON_CF_DAY_OF_MONTH, offset);
        MKTIME(calendar);
...
#define RI(field, expr_field, min, max, nextField)                                                                     \
    value = *get_field_ptr(calendar, field);                                                                           \
    update_value = find_nextprev(expr_field, max, value, min, calendar, field, nextField, resets, offset);
#define RF(field)                                                                                                      \
    if (update_value < 0)                                                                                              \
        break;                                                                                                         \
    if (value == update_value)                                                                                         \
    cron_set_bit(resets, field)
...
static int do_nextprev(cron_expr *expr, struct tm *calendar, int dot, int offset) {
    ...
    for (;;) {
        *resets = 0;
        ...
        RF(CRON_CF_HOUR_OF_DAY);
        else continue;
        value = *get_field_ptr(calendar, CRON_CF_DAY_OF_MONTH);
        update_value = find_day(calendar, expr->days_of_month, expr->day_in_month, value, expr->days_of_week,
                                calendar->tm_wday, expr->flags, resets, offset);
        RF(CRON_CF_DAY_OF_MONTH);
        else continue;
        ...
        if (update_value < 0 || value == update_value)
            break;
#endif
    }
...
static int generate_field(char *dest, uint8_t *bits, int min, int max, int offset, int overflow, int buffer_len) {
    ...
    if (n == max - min)
        ...
        goto values;
    else if (fs == min && ls + d > max - 1)
        len += sprintf(buf, "*/%d", d);
    else if (n < 3)
        goto values;
    else if (((max + overflow - fs - 1) / d) + 1 != n)
        len += sprintf(buf, "%d-%d/%d", fs, ls, d);
    else
        len += sprintf(buf, "%d/%d", fs, d);
    STRCATC(dest, buf, 0);
    return len;
values:
    from = -1;
    for (i = fs; i <= max; i++) {
        if (i < max ? cron_get_bit(bits, i + offset) : 0) {
            if (from == -1)
                from = i;
        } else if (from != -1) {
            if (!first)
...
int cron_generate_expr(cron_expr *source, char *buffer, int buffer_len, int cron_len, const char **error) {
    ...
    *buffer = '\0';
    ...
    if (cron_get_bit(source->flags, 0)) {
        ...
        STRCATC(buffer, buf, sprintf(buf, "%dW", *source->day_in_month));
    else if (*source->day_in_month != 0)
        STRCATC(buffer, "?", 1);
    else
        GFC(buffer, source->days_of_month, 1, CRON_MAX_DAYS_OF_MONTH, 0, 0, buffer_len - len);
    STRCATC(buffer, " ", 1);
    ...
    if (*source->flags)
        ...
        if (*source->day_in_month != 0) {
            ...
            if (*source->day_in_month == -1)
                ...
                STRCATC(buffer, buf, sprintf(buf, "#%d", *source->day_in_month));
        } else
            GFC(buffer, &days_of_week, 0, CRON_MAX_DAYS_OF_WEEK, 0, 1, buffer_len - len);
    }
#ifndef CRON_DISABLE_YEARS
...
static time_t cron(cron_expr *expr, time_t date, int offset) {
    ...
    if (CRON_INVALID_INSTANT == original)
        goto return_error;
    if (0 != do_nextprev(expr, calendar, calendar->tm_year, offset))
        goto return_error;
    ...
    if (calculated == original) {
        ...
        add_to_field(calendar, CRON_CF_SECOND, offset);
        MKTIME(calendar);
        if (0 != do_nextprev(expr, calendar, calendar->tm_year, offset))
            goto return_error;
    }
...
void cron_parse_expr(const char *expression, cron_expr *target, const char **error) {
    ...
    *error = NULL;
    if (!expression)
        CRON_ERROR("Invalid NULL expression");
    if (!target)
        CRON_ERROR("Invalid NULL target");
    if ('@' == *expression) {
        expression++;
        if (!strcmp("annually", expression) || !strcmp("yearly", expression))
            ...
            CRON_ERROR("@reboot not implemented");
    }
    len = count_fields(expression, ' ');
    if (len < 5 || len > 7)
        CRON_ERROR("Invalid number of fields, expression must consist of 5-7 fields");
